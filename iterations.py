...
from numpy.random import normal as awgn
...
def encoder(H, Z, SNR, d, L = 300, rv = 1, seed = 2021):
    ...
    e = awgn(loc = 0, size = x.shape, scale = sigma)
    
    y = x + e
    return y

def decoder(H, y, Z, SNR, d, tx, rv= 1, alg = 'MSA'):
    sigma = 10 ** (- SNR / 20)
    l = 2 * y / (sigma**2)
    ...
    if alg == 'MSA':
        b = MSA(H, r, d, tx, SNR = SNR)
    elif alg == 'SPA':
        b = SPA(H, r, d, tx, SNR = SNR)
    else:
...
def SPA(H, r, d, tx, SNR, max_iter = 30, threshold = 1.):
    ...
    for k in range(max_iter):
        ...
        res = '{}\t{}\t{}\t{}\t{:.3f}\t{:.6f}\t{:.6f}\t{}\t{}\n'.format(k, sum(H @ demodulate(belief) % 2), SNR, 0, 0.333, abs(sum(d - u)/len(d)), abs(sum(rx - tx)/len(tx)), sum(d - u) == 0, sum(rx - tx) == 0)
        f.write(res)
        #print(sum(H @ demodulate(belief) % 2), k)
        M = (H != 0) * (belief - E)
        #print(k, sum(H @ demodulate(belief) % 2))
        if (sum(H @ demodulate(belief) % 2) == 0):
            for k2 in range(k+1, max_iter):
                res = '{}\t{}\t{}\t{}\t{:.3f}\t{:.6f}\t{:.6f}\t{}\t{}\n'.format(k2, sum(H @ demodulate(belief) % 2), SNR, 0, 0.333, abs(sum(d - u)/len(d)), abs(sum(rx - tx)/len(tx)), sum(d - u) == 0, sum(rx - tx) == 0)
                f.write(res)
            return belief
...
def MSA(H, r, d, tx, SNR, max_iter = 30, threshold = 1.):
    belief = r
...
class experiment:
    ...
    def run(self, seed, rv):
        ...
        u = decoder(self.H, y, self.Z, self.SNR, d, tx, rv = rv, alg = self.alg)
        rx = u[:self.F.shape[1]]
...
f = open("iterations_SPA_{:.2f}_{:.3f}_{}.txt".format(SNR, R, rv), "a")
